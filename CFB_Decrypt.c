...
void CFBDecryption(FILE * filePointer, Word * KeyWords[])
{
	...
	// "output_cfb.txt" file is opened in read mode, which contains the cipher text
	filePointer = fopen("output_cfb.txt", "r");
	if (filePointer == NULL)
	{
    ...
    // "output1_cfb.txt" file is opened in write mode, which will contain the plain text
    FILE * OutfilePointer = fopen("output1_cfb.txt","w");
	...
	while ((count = fread(buffer, 1, 16, filePointer)) == 16)
	{
        ...
        for (i = 0; i < 4; ++i)
		{
            for (j = 0; j < 4; ++j)
            {
                fprintf(OutfilePointer,"%c",msg[i][j] ^ buffer[index]);
...
int main()
{
    ...
    // File pointer
	FILE* filePointer;
    CFBDecryption(filePointer, KeyWords);
